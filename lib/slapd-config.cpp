...
bool OlcDatabase::isBdbDatabase( const LDAPEntry& e )
{
    ...
    for( StringList::const_iterator i = oc.begin(); i != oc.end(); i++ )
    {
        if ( strCaseIgnoreEquals(*i, "olcBdbConfig" ) || strCaseIgnoreEquals(*i, "olcHdbConfig" ) || strCaseIgnoreEquals(*i, "olcMdbConfig" ) )
        {
            return true;
        }
    }
...
{ 
    ...
    {
        m_dbEntryChanged.addAttribute(LDAPAttribute("objectclass", "olcMdbConfig"));
    }
    else
    {
